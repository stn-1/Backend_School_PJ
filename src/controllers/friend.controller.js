  ...
  try {
    ...
    try {
      ...
      if (err.code === 11000) {
        ...
        if (existing.status === 'pending') return res.status(400).json({ message: "Đã có lời mời đang chờ xử lý." });
        if (existing.status === 'accepted') return res.status(400).json({ message: "Hai người đã là bạn bè." });
        if (existing.status === 'blocked') return res.status(400).json({ message: "Không thể gửi lời mời (Blocked)." });
      }
...
export const acceptFriendRequest = async (req, res) => {
  try {
    ...
    if (!friendship) return res.status(404).json({ message: "Không tìm thấy lời mời" });
    ...
    if (friendship.requester.toString() === userId.toString()) {
      return res.status(403).json({ message: "Bạn không thể tự chấp nhận lời mời của mình" });
    }
...
export const removeFriendship = async (req, res) => {
  try {
    ...
    const friendship = await Friendship.findById(friendshipId);
    if (!friendship) return res.status(404).json({ message: "Không tìm thấy dữ liệu" });
...
export const getFriendRequests = async (req, res) => {
  try {
    ...
    const requests = await Friendship.find({
      ...
      status: 'pending',
      requester: { $ne: userId } // $ne: Not Equal (Không phải người gửi)
    })
    .populate('requester', 'username avatar email'); // Lấy thông tin người gửi để hiển thị
